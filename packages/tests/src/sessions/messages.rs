...
use cw_auths::{
    saa_types::{
        ...
        msgs::{AllowedActions, ActionDerivation, Action}
    }, 
    SessionActionMsg, CreateSession, CreateSessionFromMsg,
};
...
fn simple_create_session_messages() {
    ...
    assert_eq!(
        Action::with_serde_name("not an object".to_string()).unwrap_err().to_string(), 
        SessionError::DerivationError.to_string()
    );
...
fn nested_session_message_checks() {
    ...
    let create_from_nested_create = CreateSessionFromMsg {
        derivation: Some(ActionDerivation::Name),
    ...
    use SessionError::{InnerSessionAction, InvalidActions};
    ...
    assert_eq!(create_from_error(&create_from_nested_create), InvalidActions);
    assert_eq!(create_from_error(&create_from_nested_from), InvalidActions);
